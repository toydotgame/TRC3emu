...
public class InstructionValidator {	
	public static String main(String line) {
		Integer opcode = OldUtils.getOpcode(line);
		Integer type = OldUtils.getType(opcode);
		if(type == null) { // Being a little over-protectice and safe with this handling but idc
			OldUtils.printAssemblerSyntaxErr(line, "Instruction type not set for the instruction \"" + line.split(" ")[0] + "\"! (Opcode " + opcode + ")");
		...
		switch(type) {
			...
			default:
				...
				OldUtils.printAssemblerSyntaxErr(line, "Type constant " + type + " invalid!");
				System.exit(2);
	...
	private static String validateALU(int opcode, int[] args) {
		if(opcode != 11 && failArgsLength(3, args)) return null;
			...
			OldUtils.printAssemberSyntaxErr("Wrong number of arguments for instruction! Should be 2 or 3, found " + args.length + ".");
			return null;
	...
	private static String validateImm3OrReg(int opcode, int[] args) {
		if(args.length == 1) {
			...
			OldUtils.printAssemberSyntaxErr("Wrong number of arguments for instruction! Should be 1 or 2, found " + args.length + ".");
			return null;
		}
	...
	private static boolean failArgsLength(int length, int[] args) {
		...
		OldUtils.printAssemberSyntaxErr("Wrong number of arguments for instruction! Should be " + length + ", found " + args.length + ".");
		return true;
	}
	...
	private static boolean failUnderOverflow(int max, int[] args) {
		for(int i = 0; i < args.length; i++) {
			if(args[i] < 0 || args[i] > max) {
				OldUtils.printAssemberSyntaxErr(args[i] + " out of bounds! Should be within 0-" + max + " inclusive.");
				return true;
			}
		}
